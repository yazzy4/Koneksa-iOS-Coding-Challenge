...
class AccelerometerMonitor: NSObject, ObservableObject {
    ...
    func startUpdate(_ freq: Double) {
        ...
        self.timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(startMotionSensor), userInfo: nil, repeats: true)
    }

        func stopMotionSensor(){
            self.timer.invalidate()
            if self.motionManager!.isAccelerometerActive && counter == 10.0 {
